...
def calculate_sunrise_sunset(date, lat, lon):
    ...
    location = LocationInfo(
        latitude=lat, longitude=lon, timezone="UTC"  # We'll work in UTC
    )
...
def annotate_daylight_periods(df, lat, lon):
    ...
    df["daylight_period"] = df.apply(
        lambda row: classify_daylight_period(
            row["datetime"], row["sunrise_utc"], row["sunset_utc"]
        ),
        axis=1,
    )
...
def analyze_solar_by_daylight_period(df):
    ...
    for period in periods:
        ...
        if len(period_data) > 0:
            ...
            print(
                f"{period.title():6s}: {avg_solar:6.1f} W/m² avg, {max_solar:6.1f} W/m² max, "
    ...
    print(
        f"  Valid daytime solar: {day_total - day_zeros:,} ({(day_total-day_zeros)/day_total*100:.1f}%)"
    )

    if day_zeros > 0:
        print(
            f"  ⚠️  Unexpected zeros during daytime - possible sensor issues or clouds"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate solar data with daylight periods"
    )
    parser.add_argument("--data-file", type=str, help="Local parquet file (optional)")
    parser.add_argument(
        "--lat", type=float, default=LATITUDE, help="Latitude (default: SF)"
    )
    parser.add_argument(
        "--lon", type=float, default=LONGITUDE, help="Longitude (default: SF)"
    )
    ...
    try:
        ...
        print(
            f"📊 Analyzing {len(df)} records from {df['dateutc'].min()} to {df['dateutc'].max()}"
        )
