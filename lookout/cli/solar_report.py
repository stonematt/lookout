...
from lookout.core.solar_analysis import (
    LOCATION, calculate_daily_energy, get_solar_statistics,
    get_seasonal_breakdown, get_weekly_analysis
)
...
def analyze_by_week(df: pd.DataFrame, daily_energy: pd.Series) -> None:
    ...
    print(f"Weekly average energy: {weekly_data['overall_avg_kwh_per_m2']:.2f} kWh/m²/day")
    print(f"Weekly peak energy: {weekly_data['overall_peak_kwh_per_m2']:.2f} kWh/m²/day")
    print()
...
def analyze_by_season(df: pd.DataFrame, daily_energy: pd.Series) -> None:
    ...
    for season in ["winter", "spring", "summer", "fall"]:
        if season in seasonal_data:
            ...
            print(f"{season_name:6s}: {data['mean_kwh_per_m2']:.2f} kWh/m²/day avg, {data['max_kwh_per_m2']:.2f} kWh/m²/day max ({data['days']} days)")
...
def print_system_design_specs(df: pd.DataFrame, daily_energy: pd.Series) -> None:
    ...
    print(f"  • Expect {stats['avg_daily_energy_kwh_per_m2']:.2f} kWh/m² per day average")
...
def print_data_quality(df: pd.DataFrame, daily_energy: pd.Series) -> None:
    ...
    print(f"Data coverage: {len(daily_energy)/((df['date'].max() - df['date'].min()).days + 1)*100:.1f}%")
    print()
    ...
    if len(daytime_df) > 1:
        ...
        print(f"  Gaps > 10min: {(intervals > 10).sum()} ({(intervals > 10).sum()/len(intervals)*100:.1f}%)")
    print()
...
def main():
    ...
    try:
        ...
        print(f"🌐 Coordinates: {LOCATION['latitude']:.3f}°N, {abs(LOCATION['longitude']):.3f}°W")
