...
def analyze_outage_gap(df, outage_date_str, outage_time_str, window_hours=4):
    ...
    outage_dt = pd.to_datetime(f"{outage_date_str} {outage_time_str}").tz_localize('America/Los_Angeles')
    ...
    if 'datetime' not in df.columns:
        df['datetime'] = pd.to_datetime(df['dateutc'], unit='ms', utc=True).dt.tz_convert('America/Los_Angeles')
    
    window_data = df[(df['datetime'] >= start_window) & (df['datetime'] <= end_window)].copy()
    ...
    for idx, row in significant_gaps.iterrows():
        gap_start = window_data.loc[window_data.index < idx, 'datetime'].iloc[-1] if len(window_data.loc[window_data.index < idx]) > 0 else 'Start'
    ...
    if len(post_outage) > 0:
        ...
        if last_before is not None:
            ...
            if pre_event > 0 and post_event == 0:
                print(f"  ❗ LIKELY EVENT LOST: Event was active before outage, reset after")
    ...
    if len(pre_outage) >= 5:
        ...
        for _, row in pre_outage.tail(5).iterrows():
            print(f"  {row['datetime']} | event:{row.get('eventrainin', 'N/A'):>6} | hourly:{row.get('hourlyrainin', 'N/A'):>6}")
