...
def calculate_daily_energy(df: pd.DataFrame) -> pd.Series:
    ...
    for date_val, group in daytime_df.groupby("date"):
        ...
        energy_wh = (
            (data["solarradiation"].shift(1) + data["solarradiation"]) / 2 * time_diffs
        ).sum()
...
def get_solar_statistics(
    df: pd.DataFrame, daily_energy: pd.Series = None
) -> Dict[str, Any]:
    """
    ...
    hourly_avg = daytime_df.groupby(daytime_df["datetime"].dt.hour)[
        "solarradiation"
    ].mean()
...
def get_seasonal_breakdown(daily_energy: pd.Series) -> Dict[str, Dict[str, float]]:
    ...
    seasonal_stats = energy_df.groupby("season")["daily_kwh_per_m2"].agg(
        ["mean", "max", "count"]
    )
    ...
    if "summer" in result and "winter" in result:
        result["seasonal_variation_ratio"] = (
            result["summer"]["mean_kwh_per_m2"] / result["winter"]["mean_kwh_per_m2"]
        )
...
def get_weekly_analysis(daily_energy: pd.Series) -> Dict[str, Any]:
    ...
    energy_df["year_week"] = (
        ...
        + "-W"
        + energy_df["week"].astype(str).str.zfill(2)
    )

    # Weekly statistics
    weekly_stats = energy_df.groupby("year_week")["daily_kwh_per_m2"].agg(
        ["mean", "max", "count"]
    )
