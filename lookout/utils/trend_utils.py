...
def get_historical_value(df: pd.DataFrame, target_time: int, field: str) -> Optional[float]:
    """
    ...
    if time_diff > max_time_diff:
        logger.debug(f"No historical data found within ±30min for {field} at target time")
        return None
...
def calculate_temperature_trend(df: pd.DataFrame, current_temp: float) -> str:
    ...
    if temp_diff > 1.0:
        trend = "↑"
        logger.debug(f"Temperature rising: {current_temp:.1f}°F vs {temp_24h_ago:.1f}°F (diff: {temp_diff:+.1f})")
    elif temp_diff < -1.0:
        trend = "↓"
        logger.debug(f"Temperature falling: {current_temp:.1f}°F vs {temp_24h_ago:.1f}°F (diff: {temp_diff:+.1f})")
    else:
        trend = "→"
        logger.debug(f"Temperature steady: {current_temp:.1f}°F vs {temp_24h_ago:.1f}°F (diff: {temp_diff:+.1f})")
...
def calculate_barometer_trend(df: pd.DataFrame, current_pressure: float) -> str:
    ...
    if pressure_change > threshold:
        trend = "↑"
        logger.debug(f"Pressure rising: {current_pressure:.2f}\" vs {pressure_3h_ago:.2f}\" (rate: {pressure_change:+.3f}\"/hr)")
    elif pressure_change < -threshold:
        trend = "↓"
        logger.debug(f"Pressure falling: {current_pressure:.2f}\" vs {pressure_3h_ago:.2f}\" (rate: {pressure_change:+.3f}\"/hr)")
    else:
        trend = "→"
        logger.debug(f"Pressure steady: {current_pressure:.2f}\" vs {pressure_3h_ago:.2f}\" (rate: {pressure_change:+.3f}\"/hr)")
