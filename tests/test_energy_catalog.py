...
class TestEnergyCatalog:
    ...
    def sample_data(self):
        ...
        data = {
            ...
            "dateutc": [int(dt.timestamp() * 1000) for dt in dates],  # Convert to milliseconds
            "solarradiation": [i % 1000 for i in range(100)]  # Mock solar radiation values
        }
    ...
    def test_catalog_exists_true(self, mock_get_client, catalog):
        ...
        mock_client.head_object.return_value = {"LastModified": datetime.now(timezone.utc)}

        assert catalog.catalog_exists() is True
        mock_client.head_object.assert_called_once_with(Bucket="lookout", Key="TEST_MAC.energy_catalog.parquet")
    ...
    def test_update_catalog_with_new_data(self, catalog, sample_data):
        ...
        existing_catalog = catalog.detect_and_calculate_periods(existing_data, auto_save=False)
