...
class TestRefactoredFunctions:
    ...
    def test_create_event_accumulation_chart_basic(self):
        ...
        fig = create_event_accumulation_chart(
            self.sample_event_data, self.sample_event_info
        )
    ...
    def test_create_event_accumulation_chart_empty_data(self):
        ...
        empty_data = pd.DataFrame({"dateutc": [], "eventrainin": [], "dailyrainin": []})
        ...
        single_point_data = pd.DataFrame(
            {
                "dateutc": [
                    int(
                        datetime(2024, 1, 15, 10, 0, 0, tzinfo=timezone.utc).timestamp()
                        * 1000
                    )
                ],
                "eventrainin": [0.1],
...
class TestEventRateChart:
    ...
    def setup_method(self):
        ...
        timestamps_ms = [
            ...
            + i * 1800000  # 30-minute intervals (gaps)
            for i in range(4)
        ]
